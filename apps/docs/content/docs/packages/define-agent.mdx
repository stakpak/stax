...
<Callout type="warn" title="Pinned References">
  Semver ranges and floating selectors like `latest` should not be used in committed manifests.
  Lockfile pinning is the intended model, but the current reference CLI does not yet write
  `stax.lock` automatically.
</Callout>
...
`stax.lock` is shown here as the intended lockfile. The current reference CLI does not yet emit it automatically during builds.
