---
title: stax pull
description: Download an artifact from a registry into the local cache
---

Pull an artifact from an OCI registry into your local cache. Once cached, the artifact can be inspected, materialized, or extracted without additional network requests.
...
Pulled artifacts are stored in the local cache at `~/.stax/cache/`, indexed by content digest:
...
```text
↓ pulling ghcr.io/myorg/agents/backend-engineer:3.1.0
  ↓ 3 new layers (8.1 KB), 4 cached
✓ pulled — sha256:a3f8c1…e7d2
```
